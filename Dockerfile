ARG SHELFY_RUNTIME_BASE_IMAGE=shelfy-runtime-base:vosk-lib-0.3.45-small-ru-0.22

FROM ${SHELFY_RUNTIME_BASE_IMAGE} AS runtime-base
...
ARG TARGETOS=linux
ARG TARGETARCH
COPY --from=runtime-base /usr/local/include/vosk_api.h /usr/local/include/vosk_api.h
COPY --from=runtime-base /usr/local/lib/libvosk.so /usr/local/lib/libvosk.so
COPY go.mod go.sum ./
RUN go mod download
COPY . .
...
RUN CGO_ENABLED=1 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -o /out/vosk-transcribe ./cmd/vosk-transcribe
